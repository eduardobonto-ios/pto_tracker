...
export function EmailPreview({ request }: { request: PTORequest }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slateish-200/80 bg-white shadow-card">
      {/* Envelope metadata */}
      <div className="space-y-1.5 border-b border-slateish-200/70 bg-slateish-50/70 px-5 py-4 text-[12.5px] sm:px-6">
        <MetaRow label="To">
      ...
      <div className="bg-canvas p-4 sm:p-8">
        <div className="mx-auto max-w-[560px] overflow-hidden rounded-2xl border border-slateish-200 bg-white shadow-card">
          <div className="bg-[radial-gradient(120%_140%_at_100%_0%,#152238_0%,#0D1729_60%)] px-6 py-6">
          ...
          <div className="px-6 py-6">
            <p className="text-[14px] leading-relaxed text-slateish-700">
              A new leave request has been filed and is pending your review:
            </p>

            <div className="mt-5 space-y-2.5 rounded-xl border border-slateish-200 bg-slateish-50/70 p-4">
            ...
            <p className="mt-6 text-[14px] leading-relaxed text-slateish-700">
              Open the PTO Tracker to approve or decline:
            </p>

            <Link
              to={`/requests/${request.id}`}
              className="mt-4 inline-flex h-11 items-center justify-center rounded-xl bg-gradient-to-b from-brand-400 to-brand-500 px-6 text-sm font-semibold text-white shadow-brand-sm transition-all hover:from-brand-500 hover:to-brand-600"
            >
              View PTO Request
            </Link>

            <p className="mt-5 break-all font-mono text-[11px] text-slateish-400">
              https://pto.valveman.com/requests/{request.id}
            </p>
          </div>

          <div className="border-t border-slateish-200 px-6 py-4">
            <p className="text-[11.5px] leading-relaxed text-slateish-400">
